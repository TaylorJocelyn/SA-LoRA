...
from quant_scripts.ddim.quant_layer import QuantModule, StraightThrough, QuantModule_intnlora
...
class QuantModel_intnlora(nn.Module):
    ...
    def __init__(self, model: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}, num_steps=100):
        ...
        self.count = 0
        self.total_count = 113 ## 73 for lsun-bedroom model, 109 for lsun-church model, 265 for imagenet, 113 for cifar10 model
        self.special_module_count_list = [1, 2, 3, 113] ## modify here for different dataset (imagenet or lsun)
        self.quant_module_refactor(self.model, weight_quant_params, act_quant_params, '')

    def quant_module_refactor(self, module: nn.Module, weight_quant_params: dict = {}, act_quant_params: dict = {}, fname: str = ''):
        """
        Recursively replace the normal conv2d and Linear layer to QuantModule
        :param module: nn.Module with nn.Conv2d or nn.Linear in its children
        :param weight_quant_params: quantization parameters like n_bits for weight quantizer
        ...
        for name, child_module in module.named_children():
            if isinstance(child_module, (nn.Conv2d, nn.Linear)) and 'skip' not in name and 'op' not in name:
                ...
                self.quant_module_refactor(child_module, weight_quant_params, act_quant_params, fname + '.' + name)
