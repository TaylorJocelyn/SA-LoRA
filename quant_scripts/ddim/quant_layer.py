...
class SimpleDequantizer(nn.Module):
    ...
    def __init__(self, uaq: UniformAffineQuantizer, weight, device):
        ...
        if len(weight.shape) == 4:
            self.delta = nn.Parameter(torch.randn(size=(weight.shape[0]*self.size_scale, 1, 1, 1), device=self.device), requires_grad=False)
            self.zero_point = nn.Parameter(torch.randn(size=(weight.shape[0]*self.size_scale, 1, 1, 1), device=self.device), requires_grad=False)
        elif len(weight.shape) == 2:
            self.delta = nn.Parameter(torch.randn(size=(weight.shape[0]*self.size_scale, 1), device=self.device), requires_grad=False)
            self.zero_point = nn.Parameter(torch.randn(size=(weight.shape[0]*self.size_scale, 1), device=self.device), requires_grad=False)
        else:
            print(weight.shape)
            raise ValueError('shape not implemented')

        self.gap = torch.tensor(list(range(0, 8, self.n_bits)), dtype=torch.uint8, device=self.device).unsqueeze(0)
