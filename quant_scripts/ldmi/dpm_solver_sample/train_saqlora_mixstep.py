...
from quant_scripts.ldmi.quant_layer import TemporalActivationQuantizer

def modify_step_size(module, new_step_size):
    for name, sub_module in module.named_children():
        if isinstance(sub_module, TemporalActivationQuantizer):
            sub_module.step_size = new_step_size
        else:
            # 递归调用，遍历更深层次的子模块
            modify_step_size(sub_module, new_step_size)
...
if __name__ == '__main__':
    ...
    lr_scheduler = get_linear_schedule_with_warmup(
        optimizer=optimizer,
        num_warmup_steps=0,
        num_training_steps=(NUM_EPOCHS * sample_steps * 2),
    )
    ...
    fp_order = 2
    quant_order = 1
    fp_steps = [200, 100, 40, 20]
    quant_steps = [100, 50, 20, 10]
    step_sizes = [1, 2, 5, 10]
    ...
    with model.ema_scope():
        for epoch in range(NUM_EPOCHS):
            for idx in range(len(fp_steps)):
                fp_step = fp_steps[idx]
                quant_step = quant_steps[idx]
                step_size = step_sizes[idx]

                # update 
                modify_step_size(model, step_size)
                ...
                if epoch % 16 == 0:
                    ...
                    image_to_save.save(os.path.join(eval_out_dir, "epoch{}_{}steps.jpg".format(epoch, quant_step)))
                    all_samples.clear()

    save_dir = "reproduce/ldmi/weight/saqlora"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    save_path = os.path.join(save_dir, 'quantw{}a{}_{}steps_saqlora_{}epochs_mixstep.pth'.format(n_bits_w, n_bits_a, sample_steps, NUM_EPOCHS))
