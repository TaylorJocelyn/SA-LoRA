...
class Diffusion(object):
        ...
        with torch.no_grad():
            ...
            with amp.autocast(enabled=False):
                if self.args.sample_type == 'generalized':
                    ...
                    x, intermediates, x_interms, t_interms = self.sample_image(x, model, last=False)
            x = inverse_data_transform(config, x)

        return x

    def sample_train(self, fp_model, q_model, batch_size):
        config = self.config
    ...
    def sample_image(self, x, model, last=True):
        ...
        if self.args.sample_type == "generalized":
            ...
            dpm_solver = DPM_Solver(model_fn, noise_schedule, algorithm_type=self.args.algorithm_type)
            return dpm_solver.sample(
                x,
                steps=self.args.timesteps,
                order=self.args.solver_order,
                skip_type="time_quadratic" if self.args.skip_type == 'quad' else 'time_uniform',
                method="singlestep",
    ...
    def collect_intermediate_data_ddim_sampler(self):
        # collect trajectories with specified sampling methods and steps for calibration 
        model = Model(self.config)
        ...
        with torch.no_grad():
            for i in tqdm.tqdm(
                ...
                if sample_num + n + 1 > self.args.cali_sample_num:
                    assert(i == epochs - 1)
                    n = self.args.cali_sample_num - sample_num - 1

                xt_list.extend(x[0][:-1])
                t_list.extend([torch.tensor(t, device=self.device) for t in x[2]])

        return xt_list, t_list
    
    def collect_intermediate_data_dpm_solver(self):
        # collect trajectories with specified sampling methods and steps for calibration 
        model = Model(self.config)
        ...
        with torch.no_grad():
            for i in tqdm.tqdm(
                ...
                if self.args.solver_order == 1:
                    xt_list.extend(x_interms[:-1])
                elif self.args.solver_order == 2:
...
class DiffusionTrainer(object):
    def __init__(self, args, config, device, fp_model, quant_model, lr_scheduler, optimizer):
        self.args = args
        self.config = config
        if device is None:
            device = (
    ...
    def generalized_steps(self, x, seq, b, train_mode=False, **kwargs):
        if train_mode:
            n = x.size(0)
            ...
            xs = [x]
            ts = []
            for i, j in tqdm.tqdm(zip(reversed(seq), reversed(seq_next)), total=len(seq), desc="Processing"):
                t = (torch.ones(n) * i).to(x.device)
                next_t = (torch.ones(n) * j).to(x.device)
                at = self.compute_alpha(b, t.long())
                at_next = self.compute_alpha(b, next_t.long())
                xt = xs[-1].to(self.device)
                et = self.fp_model(xt, t)
    ...
    def ddpm_steps(self, x, seq, b, **kwargs):
        with torch.no_grad():
            n = x.size(0)
            seq_next = [-1] + list(seq[:-1])
            xs = [x]
            ...
            for i, j in zip(reversed(seq), reversed(seq_next)):
                ...
                output = self.fp_model(x, t.float())
                quant_out =self.quant_model(x, t,float())
                e = output
    ...
    def sample_image(self, x, train_mode=False):
        try:
            skip = self.args.skip
        except Exception:
            skip = 1
        ...
        if self.args.sample_type == "generalized":
            ...
            xs = self.generalized_steps(
                x, seq, betas, eta=self.args.eta, args=self.args, device=self.device, train_mode=train_mode)
            x = xs
        elif self.args.sample_type == "dpm_solver":
            logger.info(f"use dpm-solver with {self.args.timesteps} steps")
            noise_schedule = NoiseScheduleVP(schedule='discrete', betas=self.betas)
            ...
            x = ddpm_steps(x, seq, self.betas)
        else:
            raise NotImplementedError
        return x

    def sample(self, batch_size, train_mode=False):
        config = self.config
        ...
        if train_mode:
            ...
            with amp.autocast(enabled=False):
                if self.args.sample_type == 'generalized':
                    print('train ', self.args.skip_type)
                    xs, x0_preds, ts = self.sample_image(x, train_mode)
                    x = xs[-1]
                elif self.args.sample_type == 'dpm_solver':
                    x, intermediates, x_interms, t_interms = self.sample_image(x)
            # x = inverse_data_transform(config, x)
        else:
            with torch.no_grad():
                x = torch.randn(
